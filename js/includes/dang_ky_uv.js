$(document).ready(function() {
    ...
    $('.n_dky_uv_btn').click(function() {
        ...
        // validate tên
        if ($('#n_dky_name').val().trim() == '') {
            $('.n_eror_name').html('Không để trống mục này.');
            flag = false;
            $("#n_dky_name").parents('.n_dky_uv_col')[0].scrollIntoView();
        } else if (checkkitu($('#n_dky_name').val().trim()) == true) {
            $('.n_eror_name').html('Tên không chứa kí tự đặc biệt.');
            console.log('jsa');
            flag = false;
            $("#n_dky_name").parents('.n_dky_uv_col')[0].scrollIntoView();
        } else {
        ...
        if ($('#n_dky_pwd').val() == '') {
            ...
            flag = false;
        } else if (regex_pass.test($('#n_dky_pwd').val()) == false) {
            $('.n_eror_pwd').html('tối thiểu 6 ký tự gồm tối thiểu 1 text và 1 số, không chứa dấu cách');
            flag = false;
            $("#n_dky_pwd").parents('.dky_ntd_info')[0].scrollIntoView();
        } else {
            $('.n_eror_pwd').html('');
        }
        ...
        } else if (flag_email == true || flag_phone == true) {
            var data = new FormData();
            data.append('uv_name', $('#n_dky_name').val());
            data.append('uv_email', $('#n_dky_email').val());
            data.append('uv_phone', $('#n_tel').val());
            $.ajax({
                url: '/Ajax/dang_ky_uv_loi',
                type: "POST",
                dataType: "JSON",
                contentType: false,
                cache: false,
...
$("#n_dky_email").blur(function() {
    let email = $("#n_dky_email").val();
    $.ajax({
        url: '/Ajax/email_uv',
        type: "POST",
        dataType: "JSON",
...
$('#n_dky_name').blur(() => {
    let text = '';
    let name = $('#n_dky_name').val().trim().toString().split(" ");
